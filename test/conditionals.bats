...
@test "Should branch on ( : ) for non zero on stack" {
  program='d(99:66).'
  result=$(./rpnc -e "$program" 1| head -1) # 1 is true
  assert_equal $result 99
  result=$(./rpnc -e "$program" 9| head -1) # 9 is true
  assert_equal $result 99
  result=$(./rpnc -e "$program" 0| head -1) # 0 is false
  assert_equal $result 66
}

fizzbuzz() { ./rpnc -e 'd3%(5%(0:5):5%(3:35)).' $1 ; }
